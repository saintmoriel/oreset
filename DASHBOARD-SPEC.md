...
4. ~~Tester profile, My findings, Agreements v2.~~ Done. Profile (`/operator/settings`): photo (resized in the browser to a 256px square JPEG, stored inline in `users.avatar_data_url`; move to object storage when volume justifies), unique lowercase `username`, phone, location, languages, years of security testing, experience, tools, portfolio, availability; required fields marked and listed as "missing" from `profileCompleteness` in `operator.service.ts`; photo shows in both shells via `Avatar src`. My findings (`/operator/history`): each exploited decision shows the lead auditor's verdict (verified, false positive, severity adjusted), notes and lifecycle status, or "awaiting verdict" (`getMyDecisions` joins `verified_findings`). Agreements v2: five documents with versions in `apps/api/src/modules/operator/agreement-texts.ts` (Tester Agreement, Confidentiality, Code of Conduct, Data Handling, Identity and Account); each acceptance stores the exact text, sha256, version, time, IP; only the current version counts toward onboarding (`require-onboarded.ts`); changing a text means bumping its version and everyone re-accepts. Security tab hosts the shared two-factor panel; tester sign-in has the code step. Migration 0026.
