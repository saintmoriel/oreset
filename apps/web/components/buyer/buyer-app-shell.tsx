...
import { LayoutDashboard, Package, History, Menu, X, FileSearch, FlaskConical } from 'lucide-react'
...
const NAV_ITEMS = [
  { label: 'Home', href: '/buyer/home', icon: LayoutDashboard },
  { label: 'Cases', href: '/buyer/cases', icon: FileSearch },
  { label: 'Regressions', href: '/buyer/regressions', icon: FlaskConical },
  { label: 'Datasets', href: '/buyer/datasets', icon: Package },
  { label: 'Activity', href: '/buyer/activity', icon: History },
]
  ...
  return (
    <div className={cn('flex items-center gap-2.5', className)}>
      ...
      <span className="min-w-0">
        ...
        <span className="cx-meta block truncate text-navy-400">{user?.email ?? ' '}</span>
      </span>
    </div>
  )
}
...
function SidebarContent({
  ...
  return (
    <div className="flex h-full flex-col">
      <Link href="/buyer/home" className="flex items-center gap-2 px-4 py-4" aria-label="Oreset home">
  ...
  return (
    <div className="min-h-svh bg-background">
      ...
      <div className="flex h-14 items-center justify-between border-b border-border bg-navy-50 px-4 lg:hidden">
        <Link href="/buyer/home" className="flex items-center gap-2" aria-label="Oreset home">
