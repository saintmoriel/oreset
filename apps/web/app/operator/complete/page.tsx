...
function OperatorCompleteContent() {
  const params = useSearchParams()
  const approved = params.get('approved') ?? '0'
  const escalated = params.get('escalated') ?? '0'
  const rejected = params.get('rejected') ?? '0'

  return (
    <QueueShell badge="Client Queue" signOutHref="/operator" step={2}>
      <div className="card-surface-raised flex flex-col items-start gap-4 p-8 sm:p-10" role="status">
        ...
        <h1 className="text-h2 text-foreground">Placement session complete.</h1>
        <p className="text-body text-pretty text-muted-foreground">
          Escalated items are now visible in the client&apos;s Ticket Escalation Queue.
        </p>
        ...
        <div className="grid w-full grid-cols-3 gap-3">
          ...
          <div className="rounded-lg border border-warning/20 bg-warning/5 p-4 text-center">
            <p className="text-h3 text-warning">{escalated}</p>
            <p className="text-caption mt-1 text-muted-foreground">Escalated</p>
          </div>
